...
MusicSource::MusicSource(const std::string& filename)
{
	load(filename);
}
...
void MusicSource::load(const std::string& filename)
{
	...
	p_SndFile = sf_open(path, SFM_READ, &p_Sfinfo);
	if (!p_SndFile)
	{
